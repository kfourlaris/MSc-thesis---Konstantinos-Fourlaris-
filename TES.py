...
class PitThermalEnergyStorage:
    def __init__(self, name, e_min_market=1000, e_max_market=10000000,
    ...
    def add_constraints(self, model, timesteps, hp_instance, peak_demand_kw):
        ...
        for t in timesteps:
            ...
            model.addConstr(self.U_charge[t] <= hp_instance.P_cap, name=f"charging_constraint{self.name}_{t}")
            #The discharging energy at time t cannot exceed the heat peak demand power of the network
            model.addConstr(self.V_disch[t] <= peak_demand_kw, name=f"discharging_constraint{self.name}_{t}")
