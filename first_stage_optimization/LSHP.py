...
class LargeScaleHeatPump:
    def __init__(self, name, p_min_market=500, p_max_market=70000,
        ...
        """
        Args: All data from the MAN mega heat pump project
            p_min_market: Smallest available capacity (kW_th)
            p_max_market: Largest available capacity (kW_th)
            min_load_fraction: delta_k (0.15 for 15% minimum thermal output)
    ...
    def add_constraints(self, model, timesteps, cop_vector, cop_cool_vector):
        """
        Args:
            cop_vector: A list or dictionary of 8760 pre-calculated hourly COP values from config.
            cop_cool_vector: A list or dictionary of 8760 pre-calculated hourly cooling COP values from config.
        """
        ...
        for t in timesteps:
            ...
            model.addConstr(self.V_heat[t] <= self.y_heat[t] * self.p_max, name=f"big_M_up_bound_y_heat_{self.name}_{t}")
            model.addConstr(
